...
export default function Pesquisa (){
  ...
  const onPressSearch = async () => {
    ...
    try{
      await fetch('https://backend-sestante.herokuapp.com/user/search', {
        method: 'POST',
    ...
    <ScrollView>
      {searched ? results.map(results => {
        ...
        <TouchableOpacity key={results.id} onPress={() => onPressPost(results.id)}>
            <Image source={results.user.avatar.url === null ? UserBase : results.user.avatar.url} style={Css.fotoPerfilPost}/>
