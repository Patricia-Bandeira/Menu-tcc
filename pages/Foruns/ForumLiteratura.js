...
export default function ForumLiteratura() {
  const [visibleModal, setVisibleModal] = useState(false);
  const [responsePending, setResponsePending] = useState(false);
  const [preferences, setPreferences] = useState([]);
  const [keyTagPressed, setKeyTagPressed] = useState(84);
    ...
    <View style={Css.container}>
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          Literatura é uma modalidade artística que tem como matéria-prima a palavra, usada na construção de histórias ou na expressão de emoções e ideias.
          </Text>
          <Text style={styles.textUnderline}>Tags</Text>
          <View style={styles.containerTags}>
            {preferences.map((preferences) => {
              if (preferences.id === 10) {
